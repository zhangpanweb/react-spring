...
There are 5 primitives, or building blocks in react-spring currently:
...
In the view part of your component you simply wire these props in. Make sure to extend the native element you want to animate using `animated`. If you want to animate React components or styled-components, do this:
...
return <animated.div style={props}>hello</div>`}/>
    <RewindSpring>
...
return <animated.div>{props.number}</div>`}/>
    <RewindSpring>
